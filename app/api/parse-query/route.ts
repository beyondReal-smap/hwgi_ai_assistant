...
import { validateRequest, ParseQuerySchema } from "@/lib/validation";
...
[보험 약관/보상 기준 질의]
- silson_search: 실손의료비 보험 관련 질문 (면책사항, 보상범위, 보상비율, 세대별 차이, 자기부담금 등)
  예: "라식 보험 되나요?", "실손보험 면책사항", "4세대 치과 보상", "한방치료 세대별 차이", "영양주사 면책인가요?", "스쿠버다이빙 하다 다치면?", "병실료 차액 보상", "실손 보상비율", "면책 사항 알려줘"
  ※ 실손의료비, 실손보험, 실비, 면책, 보상, 약관, 세대별, 라식, 치과, 한방, 영양제, 입원비, 통원, 자기부담금 등 약관/보상 관련 키워드가 포함되면 silson_search로 분류하세요.
...
export async function POST(req: NextRequest) {
  try {
    const v = validateRequest(ParseQuerySchema, await req.json());
    if (!v.success) return NextResponse.json({ intent: "all", targetName: null, params: {}, reasoning: v.error });
    const { query } = v.data;
    if (!query?.trim()) {
      return NextResponse.json({ intent: "all", targetName: null, params: {}, reasoning: "빈 쿼리 → 전체 고객 반환" });
    }

    const raw = await callLlm(`사용자 입력: "${query}"`, {
      maxTokens: 300,
