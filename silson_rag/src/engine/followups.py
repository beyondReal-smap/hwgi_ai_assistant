...
Zero latency (no LLM call). Returns up to 3 contextual follow-up suggestions
as concrete, actionable search queries.
"""
...
def _extract_topic(query: str) -> str:
    """Extract the core topic keyword from query."""
    cleaned = query.strip()
    ...
    cleaned = re.sub(
        r"(보험|실손|실비|의료비)?\s*(되나요|나오나요|나오나|적용되나요|가능한가요|인가요|인지|할까요|일까요|해주세요|알려주세요|알려줘|확인해|볼까요|보세요|됩니까)\s*",
        "", cleaned,
    )
...
def generate_followups(
    query: str,
    hits: list,
    filters: FilterResult | Dict[str, Any],
) -> List[str]:
    ...
    # Support both FilterResult and dict
    if isinstance(filters, FilterResult):
    ...
    # Rule 4: injury -> suggest disease query
    ...
    # Rule 5: disease -> suggest injury query
    ...
    # Rule 6: deductible -> suggest coverage limit
