...
class EmbedEngine:
    """Embedding search engine with FAISS index and TF-IDF fallback."""
    ...
    def __init__(
        ...
        self.embed_dir = embed_dir or (CFG.artifact_dir / "silson_embed")
        self.embeddings_path = self.embed_dir / "embeddings.npy"
        self.faiss_index_path = self.embed_dir / "faiss.index"
        self.meta_path = self.embed_dir / "meta.json"
        self.tfidf_cache_path = self.embed_dir / "tfidf.pkl"
    ...
    def search_filtered(self, query: str, valid_indices: set[int], topk: int = 50) -> Dict[int, float]:
        """Search and return {doc_index: normalized_score} filtered to valid_indices."""
        raw_results = self.search(query, topk=topk)
        return {idx: norm_score for idx, _, norm_score in raw_results if idx in valid_indices}
